...
public class DiaryController {
    ...
    @ApiOperation(value = "일기 텍스토와 날씨를 이용해서 DB에 일기 저장", notes = "일기를 쓸 수 있는 API")
...
//        }
    ...
    List<Diary> readDiaries(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
                            ...
                            , example = "2022-02-02")LocalDate endDate){
        return diaryService.readDiaries(startDate, endDate);
    }
    @ApiOperation("선택한 일기를 수정할 수 있습니다.")
    @PutMapping("/update/diary")
    void updateDiary(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date,
                     @RequestBody String text ){
        diaryService.updateDiary(date, text);
    }
    @ApiOperation("선택한 일기를 삭제합니다.")
